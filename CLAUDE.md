...
- **Frontend — `src/`** (React + TS + Vite + Recharts). Polls `/api/snapshot` and renders the two usage gauges, the token-over-time chart, the breakdown panel, and the projects table.
...
4. **Cost weighting** (`costOf()` + `PRICING`): each record gets a `cost` = USD-equivalent using per-model, per-token-type prices, computed **at parse time and baked onto the record**. Model class is matched by substring (opus/sonnet/haiku); anything else (e.g. `<synthetic>`) costs 0.
5. **Aggregate** (`buildSnapshot()`): one combined set of `totals`, daily `series`, per-`model` breakdown, per-`project` rows, and per-`session` rows. Excluded projects are still listed in the projects table but dropped from every total/series/window/session.
